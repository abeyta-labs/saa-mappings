...
def update_shedlock_sql_support_mapping():
    ...
    # Desired file:   ../../.advisor/mappings/shedlock-sql-support.json
    ...
    target_path = os.path.join(mappings_dir, "shedlock-sql-support.json")
    generated_rel = os.path.relpath(generated_path, repo_root)

    new_slug = "shedlock-sql-support"
    ...
    try:
        ...
        # ShedLock repo itself rather than the shedlock-sql-support artifact)
        ...
        # that gets auto-added from the repo, leaving only shedlock-sql-support
        ...
        if source_path == generated_path:
            ...
            # sql-support-only content -- if HEAD already has a committed
...
if __name__ == "__main__":
    update_shedlock_sql_support_mapping()
