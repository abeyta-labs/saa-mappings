...
def update_next_rewrite_links(rewrite_dict: dict):
    ...
    versions = sorted(list(rewrite_dict.keys()),
                      key=lambda v: parse_version(v)[:2])

    for i, version in enumerate(versions):
        if i < len(versions) - 1:
            rewrite_dict[version]["nextRewrite"]["version"] = versions[i + 1]
        else:
            rewrite_dict[version]["nextRewrite"]["version"] = None
...
def main():
    ...
    try:
        ...
        # Update nextRewrite links
        if 'rewrite' in mapping_data:
            print(f"\nUpdating nextRewrite links...")
