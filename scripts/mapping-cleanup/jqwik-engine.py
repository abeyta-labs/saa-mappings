...
def update_jqwik_engine_mapping():
    ...
    # Desired file:   ../../.advisor/mappings/jqwik-engine.json
    ...
    target_path = os.path.join(mappings_dir, "jqwik-engine.json")
    generated_rel = os.path.relpath(generated_path, repo_root)

    new_slug = "jqwik-engine"
    ...
    try:
        ...
        if source_path == generated_path:
            ...
            # clobbered that in the working tree with engine-only content
            # -- if HEAD already has a committed version at this path,
            # restore it instead of deleting, so whatever legitimately
            # lives there (the base jqwik mapping, or still-unfixed sibling
            # residue) isn't lost when this gets committed.
