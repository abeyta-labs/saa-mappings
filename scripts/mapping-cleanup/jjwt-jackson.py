...
def update_jjwt_jackson_mapping():
    # 1. Dynamically calculate paths relative to the sibling .advisor directory
    # Current folder: /scripts/mapping-cleanup/
    # Generated file: ../../.advisor/mappings/jjwt.json (repo-derived name,
    #   also the legitimate output of the separate jjwt mapping)
    # Desired file:   ../../.advisor/mappings/jjwt-jackson.json
    script_dir = os.path.dirname(os.path.abspath(__file__))
    repo_root = os.path.normpath(os.path.join(script_dir, "..", ".."))
    mappings_dir = os.path.join(repo_root, ".advisor", "mappings")
    generated_path = os.path.join(mappings_dir, "jjwt.json")
    target_path = os.path.join(mappings_dir, "jjwt-jackson.json")
    generated_rel = os.path.relpath(generated_path, repo_root)

    new_slug = "jjwt-jackson"
    wrong_coordinate = "io.jsonwebtoken:jjwt-api"

    # 2. The create-mapping workflow writes its output using the repo-derived
    # name (jjwt.json). If that's present, it's this run's freshly
    ...
    try:
        ...
        # jjwt repo itself rather than the jjwt-jackson artifact)
        if data.get("slug") != new_slug:
            print(f"Updated slug to: '{new_slug}'")
            data["slug"] = new_slug

        # 4. Drop the spurious io.jsonwebtoken:jjwt-api coordinate that gets
        # auto-added from the repo, leaving only jjwt-jackson
        ...
        if source_path == generated_path:
            # jjwt.json is also the committed output of the separate jjwt
            # mapping (jjwt-mapping.yml). This run's generator just
            # clobbered that in the working tree with jackson-only content
            # -- if HEAD already has a committed version at this path,
            # restore it instead of deleting, so the real jjwt mapping
